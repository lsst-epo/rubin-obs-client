...
export async function queryAPI(query, token, previewToken, variables = {}) {
  ...
  // Could and Should add caching in the future, would need something like:
  // "Cache-Control": "public, s-maxage=10, stale-while-revalidate=59",
  // in the header if so.
  // If so, would also need to update craft-base-image CORS "Access-Control-Allow-Headers" with "Cache-Control" as well
  const headers = !token
    ? {}
    : {
