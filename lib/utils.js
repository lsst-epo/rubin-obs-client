...
export const useList = ({
  ...
  const resultsWithNoirlabReleases = useReleases(
    site,
    section,
    results?.data?.entries
  );

  if (resultsWithNoirlabReleases) {
    results = resultsWithNoirlabReleases;
  }
...
export function makeReleaseHero(images) {
  const hero = images.find((image) => image.is_main_visual);
  if (!hero) return;
