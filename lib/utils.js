...
export const getEnhancedImageAsset = (image) => {
  const globalMetadata = useGlobalData("metadata");

  const { damMetadata } = globalMetadata.find(
    (element) => image.id.toString() === element.assetId.toString()
  );

  if (damMetadata) {
    const { metadataValue: altText } = damMetadata.find(
      (element) => element.metadataKey === "altText"
    );
    const { metadataValue: thumb } = damMetadata.find(
      (element) => element.metadataKey === "thumbnailUrl"
    );

    image = { ...image, altText, thumb };
  }
