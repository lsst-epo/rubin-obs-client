...
function getGalleryTotal(data) {
  return data?.globalSet?.galleryEntryAll[0]?.cantoAlbum?.length;
}
...
export const useList = ({
  ...
  const offset = isGallery
    ? parseInt(query.page, 10)
    : getOffset(limit, query.page, showsFeatured);
  ...
  if (results?.data) {
    ...
    results.data.total = isGallery
      ? getGalleryTotal(results.data)
      : results.data?.total;
  }
