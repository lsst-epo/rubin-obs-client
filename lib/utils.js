...
export const normalizePathData = (pathnameInput) => {
  ...
  const url = new URL(pathnameInput, base);
  const { pathname, search } = url;
  const pathParams = Object.fromEntries(new URLSearchParams(search));
...
export const getCategoryGroup = (categories, group) => {
  // gallery categories come from Canto, not Craft
  if (group === "galleryTypes") return galleryCategories;
