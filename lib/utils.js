...
export const useList = ({
  ...
  const adjustedLimit = adjustLimit(limit, page, showsFeatured);
  const offset = getOffset(adjustedLimit, page, showsFeatured);
  ...
  if (entriesWithNoirlabReleases && results?.data) {
    ...
    results.isLoading = results.isLoading || isLoading;
...
export const imageShaper = (site, data, className) => {
  ...
  /**
   * To-do: Look into why the below lookup is happening when often the wrong hardcoded value is passed in for `site`
   */
  const key = site in localeKeys ? localeKeys[site] : site;

  const { metadata, url, width, height, fileInfo, additional } = data;
  ...
  /**
   * To-do: Revise query so either the 'metadata' alias is properly defined or remove alias if it's not needed
   */
  const dataSource = metadata === undefined ? additional : metadata;

  const altText = dataSource[`AltText${key}`];
  const caption = dataSource[`Caption${key}`];
