...
export const useDamAssetAsImage = (damAsset) => {
  ...
  const { height, width, damMetadata } = damAsset;

  const { metadataValue: url } =
    damMetadata.find((e) => e.metadataKey === "directImageUrl") || {};

  const { metadataValue: altText } =
    damMetadata.find((e) => e.metadataKey === "altText") || {};

  const { metadataValue: thumb } =
    damMetadata.find((e) => e.metadataKey === "thumbnailUrl") || {};
