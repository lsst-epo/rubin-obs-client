...
export function getTimes(date, lat, lng, height) {
  ...
  // var newDate = date.toLocaleString('en-GB', {timezone: tz});
  // const destOptions = {timeZone: tz, year: 'numeric', month: 'numeric', day: 'numeric', hour: 'numeric', minute: 'numeric', second: 'numeric'};
  // var newDate = /*new Date(Date.parse(*/new Intl.DateTimeFormat('default', destOptions).format(date); // ));
  // console.log('after date:', newDate);
  ...
  for (const time in result) {
    ...
    result[time] = new Date(
      Date.parse(
        // if the locale is not set to `en-US` the parse can fail
        new Intl.DateTimeFormat("en-US", options).format(result[time])
      )
    );
  }
...
export function getMoonTimes(date, lat, lng, inUTC = false) {
  const t = new Date(date);
  const tz = tzlookup(lat, lng);

  const utcOffset = inUTC ? 0 : timezoneOffset(tz);

  t.setUTCHours(utcOffset, 0, 0, 0);
